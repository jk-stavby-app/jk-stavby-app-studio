    ...
    <div className="space-y-6 md:space-y-8 animate-in slide-in-from-bottom-4 duration-500 pb-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
          ...
          <div className="bg-white p-1 rounded-full flex shadow-sm border border-slate-200 w-full sm:w-auto overflow-hidden">
            <button
              ...
              className={`flex-1 sm:min-w-[170px] px-4 md:px-6 py-2 rounded-full text-xs md:text-sm font-bold transition-all duration-300 ${
                !showAllInvoices 
                  ...
                  : 'text-[#64748B] bg-white hover:text-[#0F172A]'
              }`}
            >
              Projektové faktury
            </button>
          ...
          <button 
            onClick={exportToCSV}
            className="flex items-center justify-center gap-2 px-6 py-3 bg-white border border-slate-200 rounded-2xl text-sm md:text-base font-bold text-[#64748B] hover:bg-slate-50 transition-all shadow-sm w-full sm:w-auto"
          >
            <Download size={18} />
      ...
      {/* Table Container */}
      <div className="bento-card bg-white p-6 md:p-8 border border-slate-50">
        {loading ? (
            ...
            <table className="w-full text-left min-w-[800px]">
              <thead>
                <tr className="text-xs font-bold text-[#64748B] uppercase tracking-wider border-b border-slate-100 pb-4">
                  <th className="px-4 py-4 sticky left-0 bg-white">Faktura</th>
              ...
              <tbody className="divide-y divide-slate-50">
                {filteredInvoices.map((inv) => (
                  <tr key={inv.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-4 py-5 sticky left-0 bg-white group-hover:bg-slate-50/50">
                    ...
                    <td className="px-4 py-5">
                      ...
                      <div className="text-sm text-[#64748B] truncate max-w-[200px] font-medium">{inv.supplier_name}</div>
                    </td>
                    <td className="px-4 py-5 text-base font-bold text-right text-[#0F172A]">{formatCurrency(inv.total_amount)}</td>
                    <td className="px-4 py-5 text-base text-[#64748B] font-medium">{formatDate(inv.date_issue)}</td>
